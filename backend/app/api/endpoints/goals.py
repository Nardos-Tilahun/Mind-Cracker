...
async def get_goal_by_id(goal_id: str, db: AsyncSession = Depends(get_db)):
    
    result = await db.execute(select(Goal).where(Goal.public_id == goal_id))
...
async def create_goal(user_id: str, req: SaveGoalRequest, db: AsyncSession = Depends(get_db)):
    ...
    return {"id": new_goal.public_id, "message": "Goal saved"} 

@router.put("/goals/{goal_id}")
async def update_goal(goal_id: str, req: SaveGoalRequest, db: AsyncSession = Depends(get_db)):
...
async def delete_goal(goal_id: str, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(delete(Goal).where(Goal.public_id == goal_id))
    await db.commit()
