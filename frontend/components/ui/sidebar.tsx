...
// ... (SidebarContext, useSidebar, SidebarProvider remain UNCHANGED) ...
      ...
      >
        <SheetContent
          ...
          className={cn(
            ...
            "fixed top-16 bottom-0 h-[calc(100svh-4rem)]",
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          // CHANGED FROM fixed to sticky to respect parent container width
...
// ... (Rest of the file: Triggers, Menus, etc. remain UNCHANGED) ...
// Just ensure the exports at the bottom are there.
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const sidebarElement = document.querySelector('[data-slot="sidebar"]') as HTMLElement
    startWidthRef.current = sidebarElement ? sidebarElement.getBoundingClientRect().width : 256
  ...
  const handleMouseUp = () => {
    ...
    if (isDraggingRef.current) {
      const sidebarElement = document.querySelector('[data-slot="sidebar"]') as HTMLElement
      if (sidebarElement) {
        const finalWidth = sidebarElement.getBoundingClientRect().width
        // SAVE COOKIE
        document.cookie = `${SIDEBAR_WIDTH_COOKIE_NAME}=${finalWidth}px; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
      }
    } else {
      toggleSidebar()
    }
  ...
  const handleDoubleClick = () => {
    if (state === "expanded") {
      ...
      // SAVE COOKIE (RESET)
      document.cookie = `${SIDEBAR_WIDTH_COOKIE_NAME}=${SIDEBAR_WIDTH}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
    }
  }
