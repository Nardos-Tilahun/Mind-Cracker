...
export function AgentCard({ state, modelName, allModels, onSwitch, isLastTurn, activeModelIds = [], onStop }: Props) {
  ...
  useEffect(() => {
      ...
      if (['reasoning', 'synthesizing'].includes(state.status)) {
          interval = setInterval(() => {
              const start = state.metrics.startTime
              setElapsed(((Date.now() - start) / 1000).toFixed(1))
          }, 100)
      } else if (state.metrics.endTime) {
          setElapsed(((state.metrics.endTime - state.metrics.startTime) / 1000).toFixed(1))
      }
      return () => clearInterval(interval)
  }, [state.status, state.metrics])

  useEffect(() => {
      if ((state.status === 'reasoning' || state.status === 'synthesizing') && logRef.current) {
    ...
    <Card className={cn(
      ...
      <CardHeader className="flex flex-row items-center justify-between py-3 px-4 bg-muted/30 border-b h-14 shrink-0">
        <div className="flex items-center gap-3 overflow-hidden">
           <div className={cn("w-2.5 h-2.5 rounded-full shadow-sm shrink-0 transition-colors duration-500",
               (state.status === 'reasoning' || state.status === 'synthesizing') ? `${statusBg} animate-pulse` : statusBg
           )} />
           <div className="flex flex-col overflow-hidden">
               ...
               <div className="flex items-center gap-2 text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                   <span className={cn(statusColor, "flex items-center gap-1 transition-colors duration-300")}>
                       <StatusIcon className={cn("w-3 h-3", (state.status === 'reasoning' || state.status === 'synthesizing') && "animate-spin")} />
                       {statusText}
                   </span>
        ...
        <div className="flex items-center gap-1">
            {/* INTERRUPTION & REGENERATE CONTROLS */}
            {isRunning ? (
                <Button 
                    variant="ghost" 
                    size="icon" 
                ...
                <Button 
                    ...
                    onClick={() => onSwitch(state.modelId)}
                    title="Regenerate"
                >
            ...
            {/* DROPDOWN FOR MODEL SWITCHING (Comparison Mode) */}
            {isLastTurn && (
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-background/80"><MoreHorizontal className="w-4 h-4"/></Button>
                    </DropdownMenuTrigger>
                        ...
                        {allModels.slice(0,5).map(m => {
                            ...
                            return (
                                <DropdownMenuItem 
                                    ...
                                    disabled={isActive}
                                >
                                    <span className={cn("flex-1", isActive && "opacity-50")}>{m.name}</span>
                                    {m.id === state.modelId && <Check className="w-3 h-3 ml-2"/>}
                                </DropdownMenuItem>
                            )
                        })}
                    </DropdownMenuContent>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar bg-linear-to-b from-transparent to-background/5">
         ...
         {state.status === 'error' && (
             <div className="p-4 rounded-xl bg-destructive/5 border border-destructive/20 text-destructive text-xs flex flex-col gap-2">
                 <div className="flex items-center gap-2 font-bold"><AlertTriangle className="w-4 h-4"/> Generation Failed</div>
                 <p className="opacity-90">{state.thinking || "Unknown error."}</p>
                 <Button variant="outline" size="sm" onClick={() => onSwitch(state.modelId)} className="mt-2 border-destructive/30 hover:bg-destructive/10 w-fit">
         ...
         {/* Thinking Process */}
         {(state.thinking || state.status === 'reasoning' || state.status === 'stopped') && state.status !== 'error' && (
             <Collapsible open={isOpen} onOpenChange={setIsOpen} className="group border border-primary/10 rounded-xl bg-primary/5 overflow-hidden transition-all hover:border-primary/20">
                 ...
                 <CollapsibleContent>
                     <div ref={logRef} className="max-h-[200px] overflow-y-auto p-4 text-[11px] font-mono leading-relaxed text-muted-foreground/90 bg-background/40 border-t border-primary/10 whitespace-pre-wrap">
                         {state.thinking || <span className="animate-pulse text-primary/60">Initializing reasoning logic...</span>}
                         {state.status === 'reasoning' && <span className="inline-block w-1.5 h-3 ml-1 align-middle bg-primary animate-pulse"/>}
                     </div>
                 </CollapsibleContent>
             </Collapsible>
         )}
         ...
         {state.jsonResult?.message && (
             <div className="text-sm leading-7 text-foreground/90 p-1 animate-in fade-in slide-in-from-bottom-2">
                {state.jsonResult.message}
             </div>
         )}

         {/* Result Steps */}
         {state.jsonResult?.steps && (
            <div className={cn("space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500", state.status === 'stopped' && "opacity-80 grayscale-[0.5]")}>
                <div className="h-32 w-full bg-muted/20 rounded-xl border border-border/50 p-3">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={state.jsonResult.steps}>
                            <Tooltip
                                cursor={{fill: 'var(--muted)', opacity: 0.2}}
                                content={<CustomChartTooltip />}
                            />
                            <Bar dataKey="complexity" radius={[4,4,0,0]} animationDuration={1000}>
                                {state.jsonResult.steps.map((e:any, i:number) => (
                                    <Cell key={i} fill={e.complexity > 7 ? 'var(--color-destructive)' : 'var(--color-primary)'} opacity={0.8} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                ...
                <div className="space-y-2.5">
                    {state.jsonResult.steps.map((s: any, i: number) => (
                        <div key={i} className="flex gap-3 p-3 rounded-xl border border-border/60 bg-card/50 hover:bg-card transition-colors">
                            <Badge variant="outline" className="h-6 w-6 p-0 flex items-center justify-center shrink-0 rounded-lg bg-background">
                                {i+1}
                            </Badge>
